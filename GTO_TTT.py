...
def winning_condition(board, player):
    ...
    for i in range(3):
        if (board_numpy[i] == a).all() or (board_numpy.T[i] == a).all():
            return True
    if (np.diag(board_numpy) == a).all() or (np.diag(np.fliplr(board_numpy)) == a).all():
        return True
    return False
...
def create_board(board, zero, player):
    ...
    boardcopy[zero] = 1 if player else -1
    return boardcopy
...
def gto(board, player):
    ...
    if player:
        index = np.argmax(returns) # we used to use this, but the following code provides random choices, if the returns are equal for different spaces
    elif not player:
        index = np.argmin(returns)
